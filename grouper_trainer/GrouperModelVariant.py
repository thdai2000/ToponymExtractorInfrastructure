...
if __name__ == '__main__':
    ...
    # grouper.load_annotations_from_file("./grouper_data/grouper_data_merged.json")
    #
    ...
    #
    # train(model, train_dataloader, test_dataloader, optimizer, criterion, 10, device)
    #
    ...
    grouper.load_annotations_from_file("./grouper_data/maptext_val.json")

    test_set = grouper.get_dataset(sample_ratio=1)
    test_set = DatasetClassTrainingVariantLength(test_set)
    test_dataloader = DataLoader(test_set, batch_size=1, shuffle=False)
    # test_set = DatasetClassTrainingVariantLength(test_set)
    # test_dataloader = DataLoader(test_set, batch_size=1, shuffle=False)
    ...
    # model = GrouperTransformerEncoderDecoderAttentionNoFontPadded()
    model = torch.load('./checkpoints/grouper_model_old_epoch1.pth')
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    print('Device: {}'.format(device))
    if 'GrouperTransformerEncoderDecoder' in model.type:
